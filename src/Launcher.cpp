...
Launcher::Launcher(const std::string &client_port,
...
Protocol Launcher::makeClientProtocol()
{
    ...
    return Protocol(Listener{
        [this]() {
            ...
            std::thread([this]() {
                try
                {
                    while (true)
                    {
                        srvLolClient.send(Protocol::makePacket(Protocol::MAESTROMESSAGETYPE_HEARTBEAT,
                                                               raw_bytes()));
        ...
        [this](message &&msg) {
            ...
            Logger::log(LOG_FILE, "ReceiveMessage [", Util::getHex(command), ']');
            Logger::log(LOG_STDOUT, "ReceiveMessage [", Util::getHex(command), ']');
            switch (command)
            {
            ...
            case Protocol::MAESTROMESSAGETYPE_CHATMESSAGE_TO_GAME:
                ...
                break;
            default:
                Logger::log(LOG_STDOUT, "Client: Ignored command [", Util::getHex(command), ']');
            }
    ...
    return Protocol(Listener{
        [this]() {
            ...
            std::thread([this]() {
                try
                {
                    while (true)
                    {
                        srvLolGame.send(Protocol::makePacket(Protocol::MAESTROMESSAGETYPE_HEARTBEAT,
                                                             raw_bytes()));
                        std::this_thread::sleep_for(Protocol::HEARTBEAT_TIME_INTERVAL * 1s);
                    }
                }
        ...
        [this](message &&msg) {
            ...
            case Protocol::MAESTROMESSAGETYPE_CHATMESSAGE_FROM_GAME:
                ...
                break;
            default:
                Logger::log(LOG_STDOUT, "Game: Ignored command [", Util::getHex(command), ']');
                return Protocol::makeMsg(Protocol::MAESTROMESSAGETYPE_IGNORE, raw_bytes());
            }
            return Protocol::makeMsg(Protocol::MAESTROMESSAGETYPE_REPLY, raw_bytes());
        },
...
void Launcher::launchClient()
{
    ...
    auto proc_info = Util::createProcess(lolRootPath + "\\Air\\LolClient.exe",
                                         std::string("Air\\LolClient.exe") +
                                             " -runtime .\\ -nodebug META-INF\\AIR\\application.xml .\\" +
                                             " -- " + clientPort +
...
void Launcher::launchGame(const char *args)
{
    ...
    auto proc_info = Util::createProcess(lolRootPath + "\\Game\\League of Legends.exe",
                                         std::string("Game\\League of Legends.exe ") + gamePort +
                                             " lol.launcher_tencent.exe Air\\LolClient.exe " + args,
                                         lolRootPath);
    if (proc_info.first)
        throw Error(Util::getErrMsg(proc_info.first));
}
...
void Launcher::launch()
{
    ...
    Logger::create(LOG_FILE, new std::ofstream("LolLauncher.log", std::ios::app));
    try
    {
        WSADATA wsa;
        int err_no;
        if (err_no = WSAStartup(MAKEWORD(2, 2), &wsa))
            throw Error(Util::getErrMsg(err_no));
        srvLolClient.start();
